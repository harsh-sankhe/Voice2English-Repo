...
• **g[l]** = Activation function to be used. (Sigmoid / ReLU)  
• **z[l]** = Linear component for layer l computed as: `z[l] = w[l] * a[l-1] + b[l]`

![parameter](images/parameter.png)

For example - for the 2nd hidden layer in the neural network, the dimension of the weight matrix will be (5×3) as it depends on the number of neurons in the current layer and the number of neurons in the previous layer. The dimension of the bias vector will be (5×1) matching the number of neurons in the current layer. The input feature vector x has a dimension of (2×1) as there are two input features.

The general rule is: for any layer l, the weight matrix W[l] has shape (n[l]×n[l−1]) the bias vector b[l] has shape (n[l]×1) and the linear output Z[l]=W[l]A[l−1]+b[l] also has shape (n[l]×1)
...
It starts with the input `a[0] = x`, each layer computes a linear transformation `z[l]=W[l]a[l−1] + b[l]`, followed by a non-linear activation `a[l]=g[l](z[l])`. This continues until the final output layer, where the activation `a[L]` becomes the prediction `yhat`.
...
**Parameters** are the internal variables of a model such as weight vector (w) and bias term (b), which are learned from the training data. These are updated during the training process using forward and backward propagation.

**Hyperparameters** are external variables that controls how the model learns. They are not learned from the data but are provided before training begins. Common hyperparameters include:  
Learning rate (α), Number of iterations, Number of layers and units per layer, Activation functions, Batch size, momentum, and regularization parameters.
