...
Building Blocks of a Deep Neural Network involves Two Steps : Forward Propagation and Backward Propagation.
...
It starts with the input `a[0] = x`, each layer computes a linear operation `z[l] = W[l]a[l−1] + b[l]`, followed by a non-linear activation `a[l] = 
g[l](z[l])`. This continues until the final output layer, where the activation `a[L]` becomes the prediction `yhat`.
...
•  Gradients (partial derivatives) of the loss w.r.t each parameters are computed using the *chain rule*.  
...
• **Hyperparameters** are external variables that controls how the model learns. 
